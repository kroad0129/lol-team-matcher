...
export async function POST(req: NextRequest) {
  const { players, mode }: { players: PlayerInput[]; mode: "tier" | "power" } = await req.json()

  if (!Array.isArray(players) || players.length !== 10) {
    return NextResponse.json({ error: "플레이어 수는 정확히 10명이어야 합니다." }, { status: 400 })
  }

  const { teamA, teamB } = findBestTeams(players, mode)
  return NextResponse.json({ teamA, teamB, scoreDiff: 0 })
}

function findBestTeams(players: PlayerInput[], mode: "tier" | "power") {
  const combos = combinations(10, 5)
  let best = { score: Number.POSITIVE_INFINITY, teamA: [] as PlayerInput[], teamB: [] as PlayerInput[] }
  ...
  for (const pick of combos) {
    ...
    const skillGap = Math.abs(
      teamA.reduce((s, p) => s + power(p, mode), 0) - teamB.reduce((s, p) => s + power(p, mode), 0),
    )
...
function laneBalance(team: PlayerInput[]) {
  ...
  team.forEach((p) => {
    if (LANES.includes(p.mainLane)) map.set(p.mainLane, (map.get(p.mainLane) ?? 0) + 1)
  })

  team.forEach((p) => {
    if (LANES.includes(p.subLane) && (map.get(p.subLane) ?? 0) < 1) map.set(p.subLane, (map.get(p.subLane) ?? 0) + 0.6)
  })

  team
    .filter((p) => p.mainLane === "상관없음")
    .forEach(() => {
      const needLane = [...LANES].sort((a, b) => (map.get(a) ?? 0) - (map.get(b) ?? 0))[0]
